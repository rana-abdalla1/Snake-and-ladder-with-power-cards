...
Player* Grid::GetNextPlayer(Player* p1) 
{
	return (p1->getClosestPlayer(PlayerList, MaxPlayerCount-1));
}
...
bool Grid::IsOverlapping(GameObject* newobj) const
{
	for (int i = NumVerticalCells - 1; i >= 0; i--) // bottom up
	{
		for (int j = 0; j < NumHorizontalCells; j++) // left to right
		{
			GameObject* g1 = CellList[i][j]->GetGameObject();
			bool z = g1->IsOverlapping(newobj);
			if (z == true)
			return true;
		}
...
void Grid::resetAllPlayers()
{
	...
	for (int i = 0; i < MaxPlayerCount; i++)
	{
		PlayerList[i]->resetplayers(this);
	}
}
...
{
	int row = pos.VCell();
	int column = pos.HCell();
	return CellList[row][column]->HasCard();
}
void Grid::SaveAll(ofstream& fout, Type t)
{
	for (int i = NumVerticalCells - 1; i >= 0; i--)
	{
		for (int j = 0; j < NumHorizontalCells; j++)
		{
			if (CellList[i][j]->GetGameObject())
				CellList[i][j]->GetGameObject()->Save(fout, t);
		}
	}
}

void Grid::resetAllCities()
{
	CardNine::resetowner();
	CardTen::resetowner();
	CardEleven::resetowner();
}
void Grid::DecreaseCurrentPlayer() //Similar to advance player
{
	currPlayerNumber--;

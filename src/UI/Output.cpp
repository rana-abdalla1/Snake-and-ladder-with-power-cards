...
void Output::CreateDesignModeToolBar() const
{
	...
	MenuItemImages[ITM_COPY_CARD] = "images\\Menu_CopyCard.jpeg";
	MenuItemImages[ITM_CUT_CARD] = "images\\Menu_CutCard.jpg";
	MenuItemImages[ITM_PASTE_CARD] = "images\\Menu_PasteCard.jpg";
	MenuItemImages[ITM_DELETE_GAME_OBJECT] = "images\\Menu_DeleteGameObject.jpeg";
...
void Output::DrawLadder(const CellPosition& fromCell, const CellPosition& toCell) const
{
	...
	int numOfExtraCrossLines = (fromCell.VCell() - toCell.VCell()) - 1;
	pWind->SetPen(UI.LadderColor, UI.LadderlineWidth);
	int heightOfFirstCross = y1 - UI.LadderYOffset;
	pWind->DrawLine(x12, heightOfFirstCross, x34, heightOfFirstCross);
	int k = heightOfFirstCross - UI.CellHeight;
	for (int i = 0;i < numOfExtraCrossLines;i++)
	{
		pWind->SetPen(UI.LadderColor, UI.LadderlineWidth);
		pWind->DrawLine(x12, k, x34, k);
		k -= UI.CellHeight;
	}
...
void Output::DrawSnake(const CellPosition& fromCell, const CellPosition& toCell) const
{
	...
	int xp[4] = { x12,x12 + xChange,x12,x12 - xChange };
	int yp[4] = { y1 + yChange,y1,y1 - yChange,y1 };


	///TODO: Draw the Polygon (diamond) representing the Snake's Head
	//       Check the snakes drawn in the project document and draw it the same way

	pWind->DrawPolygon(xp, yp, 4);
